...
// Ensures each agent run's lifecycle.start event is claimed by exactly one
// concurrent inbound handler, preventing runId cross-contamination when
// multiple sessions are processed concurrently.
...
export async function handleDingTalkMessage(params: HandleDingTalkMessageParams): Promise<void> {
  ...
  try {
    ...
    const unsubscribeAgentEvents = rt.events.onAgentEvent((event: any) => {
      // Capture runId from the first unclaimed lifecycle "start" event.
      // Use claimedRunIds to avoid stealing another concurrent session's runId
      // when multiple inbound handlers are subscribed to the global event bus.
      if (!capturedRunId && event.stream === "lifecycle" && event.data?.phase === "start") {
        const runId = event.runId as string;
        if (runId && !claimedRunIds.has(runId)) {
    ...
    try {
        ...
        dispatcherOptions: {
          ...
          deliver: async (payload, info) => {
            try {
              ...
              // Use exact match only: startsWith checks are too aggressive and can
              // suppress the final reply when it begins with the same preamble text.
              const textNorm = textToSend.trim().replace(/[。.!！？?]+$/, "");
              const alreadyShown = alreadySentSnippets.some((s) => textNorm === s);
              if (alreadyShown) {
                log?.debug?.(
      ...
      if (capturedRunId) claimedRunIds.delete(capturedRunId);
